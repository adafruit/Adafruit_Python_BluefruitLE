...
def main():
    ...
    # Can search for a device with any name:
    #device = UART.find_device(timeout_sec=30)
    # Or search for a device with a specific name:
    device = UART.find_device(timeout_sec=30, name='UART')
    if device is not None:
        print 'Found device {0} with address {1}!'.format(device.name, device.address)
    else:
